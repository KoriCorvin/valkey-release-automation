...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--key-name \"Name\"] [--key-email \"email@example.com\"]"
      exit 0
      ;;
...
REPO=$(gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null) || {
  ...
  echo "Run 'gh auth login' first, then run this script from the repo root."
  exit 1
}
...
gpg --armor --export-secret-keys "${KEY_EMAIL}" | gh secret set GPG_PRIVATE_KEY
echo "Secret GPG_PRIVATE_KEY stored."

# Verify
gh secret list | grep -q GPG_PRIVATE_KEY && echo "Verified: secret exists." || {
